...
echo
printf "  %s (ENTER = %s): " "$L_USER_ASK" "${USER:-$L_ADDR_DEF}"
read -r username
[ -z "$username" ] && username="${USER:-$L_ADDR_DEF}"

defaddr="$L_ADDR_DEF"
printf "  %s (ENTER = %s): " "$L_ADDR_ASK" "$defaddr"
read -r addressing
[ -z "$addressing" ] && addressing="$defaddr"
