...
Most commands also have keyboard shortcuts using `ctrl+x` as the default leader key.
...
> **Note:** Some editors like VS Code need to be started with the `--wait` flag. Some editors need command-line arguments to run in blocking mode. The `--wait` flag makes the editor process block until closed.
