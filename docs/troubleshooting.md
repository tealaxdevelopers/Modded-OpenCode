...
The installer updates `~/.zshrc` for Zsh and `~/.bashrc` otherwise. It replaces only the block between the `# >>> opencode setup >>>` and `# <<< opencode setup <<<` markers. API keys are stored in `.env.local` (not in RC files) on macOS/Linux. On Windows, `setup.bat` uses `setx` to store keys as user environment variables — see the README for per-OS details.
