...
└── source/
    ...
    ├── rules.md                       # Правила персоны агента (своё обращение + язык)
    ├── .gitignore
    ├── agents/                        # 13 собственных агентов (ivan, scout, planner, review...)
    ├── commands/                      # 19 slash-команд
    ├── instructions/                  # 22 набора инструкций
    └── skills/                        # 105 пакетов SKILL.md
```

Плагины устанавливаются через npm (не как локальные файлы):

| Плагин | Пакет | Назначение |
|--------|-------|------------|
| **agents-opencode** | `modded-opencode-agents-opencode` | Контекст сжатия, инъекция версии, блокировка чувствительных файлов |
...
Всё остальное устанавливается автоматически: 105 навыков, 13 агентов, 19 команд, 22 набора инструкций, MCP-серверы.
...
**Включено по умолчанию.** Плагин auto-continue следит за сессиями: когда сессия **простаивает** (модель закончила, но вы не написали) **или связь обрывается посреди задачи** (`session.error`), он автоматически вставляет сообщение `continue`, и агент продолжает сам — без вашего или ИИ участия.
...
## 📜 Rules.md (Персона агента)

Сердце этого набора. `rules.md` конфигурирует слой персоны, ограниченный сессией, загружаемый через систему инструкций OpenCode — идентичность, голос и стиль работы агента:

- 🎭 **Определение персоны:** устанавливает операторскую личность агента — голос, формат ответа и стиль работы для сессии.
- 🌐 **Язык + обращение:** оба задаются при установке (`{{LANGUAGE}}`, `{{HITAP}}`) — агент говорит на вашем языке и называет вас так, как вы выбрали.
...
> **Примечание:** это конфигурация персоны для частной, контролируемой сессии — не эксплойт безопасности. Не рекомендуется для продакшена или многопользовательских систем.
...
- [awesome-opencode/awesome-opencode](https://github.com/awesome-opencode/awesome-opencode) — курируемый список плагинов, тем, агентов и ресурсов
