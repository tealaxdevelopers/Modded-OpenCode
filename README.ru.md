...
## 🔌 Плагины (6 шт., устанавливаются через npm)

| Плагин | Что делает |
|--------|-----------|
| **agents-opencode** | Инъекция контекста сжатия, version env var |
...
При выборе **локальной персоны** (по умолчанию) используется встроенный [`DEFAULT_PERSONA`](https://github.com/tealaxdevelopers/Modded-OpenCode/blob/main/scripts/build-config.mjs) из `build-config.mjs` как статья 2 файла `rules.md`. Эта персона работает офлайн без внешних зависимостей. Вы можете прочитать и отредактировать её напрямую — это обычный Markdown-промпт.
